...
export async function askAI(body, options = {}) {
  const { model, messages, max_tokens, temperature, user, ...rest } = body || {};

  // --- Validasi Input Mirip OpenAI ---
  if (!messages) {
    return Promise.reject(createOpenAIError('Missing required parameter: messages.', 'invalid_request_error', 'messages'));
  }

  if (!Array.isArray(messages)) {
    return Promise.reject(createOpenAIError('\'messages\' must be an array.', 'invalid_request_error', 'messages', 'messages_type_invalid'));
  }

  if (messages.length === 0) {
    return Promise.reject(createOpenAIError('\'messages\' array must not be empty.', 'invalid_request_error', 'messages', 'messages_empty'));
  }

  // Validasi isi messages
  for (const [index, msg] of messages.entries()) {
    if (!msg || typeof msg !== 'object') {
       return Promise.reject(createOpenAIError(`Message at index ${index} must be an object.`, 'invalid_request_error', `messages[${index}]`, 'message_not_object'));
    }
    if (!msg.role || typeof msg.role !== 'string') {
       return Promise.reject(createOpenAIError(`Message at index ${index} must have a 'role' field of type string.`, 'invalid_request_error', `messages[${index}].role`, 'message_role_missing'));
    }
    ...
    if (typeof msg.content !== 'string') { // OpenAI juga mendukung array content, tapi kita sederhanakan
       return Promise.reject(createOpenAIError(`Message at index ${index} must have a 'content' field of type string.`, 'invalid_request_error', `messages[${index}].content`, 'message_content_type_invalid'));
    }
  }
  ...
  try {
    ...
    if (typeof aiTextResponse === 'string') {
      ...
      const dummyId = `chatcmpl-${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}`;

      // Estimasi penggunaan token (sangat kasar)
      const promptTokens = estimateTokens(JSON.stringify(finalMessagesToSend));
      ...
      return Promise.reject(createOpenAIError('Invalid or missing text in API response.', 'api_error', null, 'response_text_invalid'));
    }
    ...
    if (axios.isAxiosError(error)) { // Cek jika itu error dari Axios
      if (error.code === 'ECONNABORTED') { // Timeout
        ...
        if (status === 400) {
          ...
          errorMessage = `HTTP ${status}: ${error.response.statusText || 'Error'}.`;
          errorType = 'api_error';
    ...
    return Promise.reject(createOpenAIError(errorMessage, errorType, null, errorCode));
  }
}
...
async function runExamples() {
  ...
  const requestBody1 = {
    ...
    messages: [
      { role: "system", content: "Kamu adalah ahli dalam memberikan saran percintaan. Jawab dengan bahasa Indonesia santai dan singkat." },
  ...
  const requestBody2 = {
    ...
    messages: [
      ...
      { role: "user", content: "Sebutkan 2 tempat wisata populer di sana." }
    ],
