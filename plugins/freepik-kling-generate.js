...
export default {
    ...
    description:
        "Membuat task video motion control dengan API Key acak (Auto-remove limit key)",
    method: "post",
    ...
    parameters: [
        ...
        },
        {
            ...
            type: "string",
            required: false,
            description: "image atau video (default: video)"
        }
    ],
    handler: async (req, res) => {
        try {
            const { image_url, video_url, prompt, character_orientation } =
                req.body;
            if (!image_url || !video_url) {
                return res
                    ...
                    .json({
                        success: false,
                        message: "image_url dan video_url wajib diisi"
                    });
            }
            ...
            try {
                ...
                return res
                    ...
                    .json({
                        success: false,
                        message:
                            "File API Key tidak ditemukan. Silakan tambahkan key terlebih dahulu via endpoint /api/freepik/keys."
                    });
            }
            ...
            if (keys.length === 0) {
                return res
                    ...
                    .json({
                        success: false,
                        message:
                            "Daftar API Key kosong. Silakan tambahkan key baru."
                    });
            }
                ...
                try {
                    createRes = await axios.post(
                        "https://api.freepik.com/v1/ai/video/kling-v3-motion-control-pro",
                        {
                            ...
                            prompt:
                                prompt ||
                                "follow the subject's movements and camera movements with precision. sometimes body parts that should be touching each other become spaced apart due to differences in body size, so try to follow the movements in the video, don't leave any space.",
                            //cfg_scale: 1,
            ...
            if (!createRes) {
                return res.status(400).json({
                    success: false,
                    message:
                        "❌ Semua API Key yang tersimpan telah habis limit atau tidak valid.",
                    feedback_keys_dihapus: removedKeys
                });
            }
            ...
            return res.json({
                success: true,
                message:
                    "✅ Task berhasil dibuat. Gunakan task_id dan used_api_key untuk melakukan polling status.",
                data: {
